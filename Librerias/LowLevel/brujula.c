 ...
 * @file brujula.c
 * @author Jorge Ibáñez
 * @brief Definición de los drivers de bajo nivel para el uso de la brujula HMC5883L (realmente QMC5883L).
 * @version 0.1
 * @date 2025-05-06
 * 
...
void brujula_leer(int16_t* x, int16_t* y, int16_t* z)
{
	...
	TWI_Write(0x00); // Registro de datos
	TWI_Stop();
	...
	uint8_t yl = TWI_Read_ACK();
	uint8_t yh = TWI_Read_ACK();
	uint8_t zl = TWI_Read_ACK();
	uint8_t zh = TWI_Read_NACK();
