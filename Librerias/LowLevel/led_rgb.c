...
void led_rgb_send_bit(uint8_t bit)
{
	...
	if (bit)
	{
		// send '1': mantener el pin en HIGH ~0.7 µs, luego LOW ~0.6 µs
	...
	{
		// send '0': mantener el pin en HIGH ~0.35 µs, luego LOW ~0.8 µs
		GPIO_Write(&RGB_PORT, RGB_PIN, HIGH);
		_delay_us(0.35);
		GPIO_Write(&RGB_PORT, RGB_PIN, LOW);
		_delay_us(0.8);
...
void led_rgb_send_byte(uint8_t byte)
{
	...
	{
		led_rgb_send_bit(byte & (1 << (7 - i))); // El bit actual se evalua con un AND bit a bit y se envia
	}
}
...
void led_rgb_send_color(uint8_t red, uint8_t green, uint8_t blue)
{
	...
	led_rgb_send_byte(green/10); // Valores reducidos para reducir intensidad por seguridad
	led_rgb_send_byte(red/10);
	led_rgb_send_byte(blue/10);
